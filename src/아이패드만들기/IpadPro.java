...
public class IpadPro {
    ...
    IpadPro(String name) { // 생성자 : 클래스가 객체로 만들어 질 때 호출
        this.name = name;
        Date now = new Date(); // 시간정보를 가져 오기 위한 Date 클래스를 객체로 생성
    ...
    void setColor() {
        ...
        while (true) {
            ...
            color = sc.nextInt();
            if(screen == 1 || screen == 2) return;
            System.out.print("컬러를 다시 선택 하세요. ");
        }
    }
    void setMemory() {
        Scanner sc = new Scanner(System.in);
