...
public class MethodOverLoading {
    public static void main(String[] args) {
        ...
        System.out.println(sum("seoul", "busan","cheonan"));
        System.out.println(sum(100,200, 300, 30.5));
    ...
    // 매개변수의 개수가 다름(오버로딩 가능)
    static int sum(int x, int y, int z) {
        return x + y + z;
    }
    // 매개변수의 타입이 다름(오버로딩 가능)
    static String sum(int x, String y, int z) {
        return x + y + z;
    }
    // 반환 타입은 오버로딩 규칙과 상관없으며, 매개변수의 개수가 다름
    static double sum(int x, int y, int z, double aa) {
