...
public class AirCon {
    ...
    public AirCon() { // 생성자 : 클래스가 객체가 될 때 자동으로 호출, 인스턴스 필드를 초기화하는게 주 목적
        final int[] monthTemp = {-5, 3, 8, 15, 20, 26, 32, 30, 24, 16, 9, 4};
        Calendar now = Calendar.getInstance();
        int month = now.get(Calendar.MONTH); // month 는 현재 몇월인지 정보를 가져 오는데 정보가 배열의 인덱스 같음
        currTemp = monthTemp[month];
        isPower = false; // 전원에 대한 인스턴스 필드 O
        setTemp = 18;
        isCooler = false;
        isHeater = false;
        windStep = 1;
    ...
    public void setAirConState() {
        ...
        setTemp = sc.nextInt(); // 설정 온도를 입력 받음
